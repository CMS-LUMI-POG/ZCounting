...
if __name__ == '__main__':
    ...
    parser.add_argument("--byLsCSV", help="ByLs csv input generated by testBril.sh",
                        default="default")
    parser.add_argument("--byLsCSVReplace", help="ByLs csv input generated by testBril.sh",
                        default=None)                  
    ...
    if args.byLsCSVReplace is not None:
        byLS_filelist2 = glob.glob(args.byLsCSVReplace)
        byLS_filelist2.sort(key=os.path.getmtime)
        byLS_filename2 = byLS_filelist2[-1]

        byLS_data_2 = load_input_csv(byLS_filename2)
    ...
    for run, byLS_run in byLS_data.groupby('run', sort=True):
        ...
        if args.byLsCSVReplace is not None:
            byLS_run_2 = byLS_data_2.loc[byLS_data_2['run'] == run]
        ...
        for m, goodLSlist in enumerate(
            ...
            if args.byLsCSVReplace is not None:
                byLS_m_2 = byLS_run_2.loc[byLS_run_2['ls'].isin(goodLSlist)]
            ...
            if df is None:
                ...
                if args.byLsCSVReplace is not None:
                    df_2 = df.append(byLS_m_2, sort=False)
            ...
            if not args.collect and args.byLsCSVReplace is None:
            ...
            if result is not None:          
                if args.byLsCSVReplace is not None:    
                    print(f"replace {df['delivered(/pb)'].sum()} with {df_2['delivered(/pb)'].sum()} delivered")
                    print(f"replace {df['recorded(/pb)'].sum()} with {df_2['recorded(/pb)'].sum()} recorded")
                    delLumi = df_2['delivered(/pb)'].sum()
                    recLumi = df_2['recorded(/pb)'].sum()
                    avgpu = df_2['avgpu'].mean()
                else:
